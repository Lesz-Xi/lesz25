    ...
    >
      ...
      <div className="relative w-full h-[65vh] md:h-[80vh] flex items-center justify-center max-w-[90vw] md:max-w-5xl mt-16 md:mt-20">
        {projects.map((project, index) => (
          <div
            key={project.id}
            className="project-card absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full md:w-[600px] h-[400px] md:h-auto md:aspect-[16/10] rounded-2xl md:rounded-3xl shadow-2xl flex flex-col md:flex-row bg-[#1C1C21] origin-top"
            style={{ zIndex: index + 1 }}
          >
            {/* Image Section */}
            <div 
              className="w-full md:w-2/3 h-[35%] md:h-full relative overflow-hidden rounded-t-2xl md:rounded-l-3xl md:rounded-tr-none px-4 py-4 md:px-0 md:py-0"
            ...
            {/* Content Section */}
            <div className="w-full md:w-1/3 h-[65%] md:h-full p-4 md:p-8 flex flex-col justify-between bg-[#1C1C21] border-t md:border-t-0 md:border-l border-neutral-800 rounded-b-2xl md:rounded-r-3xl md:rounded-bl-none">
               <div>
